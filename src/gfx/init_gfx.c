...
/*   Updated: 2021/02/21 15:36:56 by abrabant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_win_res(t_cub3d *c3d)
{
	...
	if (c3d->gfx.win_width > (unsigned int)screen_res_x)
		c3d->gfx.win_width = screen_res_x;
	if (c3d->gfx.win_height > (unsigned int)screen_res_y)
...
int	init_gfx(t_cub3d *c3d)
{
	...
	if (!init_img(&c3d->gfx, &c3d->mapdat))
		return (ft_snprintf(c3d->err, ERR_LEN, "Failed to init mlx imgs."));
	c3d->gfx.win_ptr = mlx_new_window(c3d->gfx.mlx_ptr, c3d->gfx.win_width,
			c3d->gfx.win_height, "cub3D");
